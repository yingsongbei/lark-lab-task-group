...
| 空间 | 主要作用 | 典型内容 |
|---|---|---|
| 私聊机器人 | 整理、预览和高影响操作确认 | 批量录入任务、修改字段、删除任务、权限调整、周报预览 |
...
```mermaid
flowchart TD
    A["老师或成员在群里提出任务"] --> B["协调人私聊机器人整理"]
    B --> C["机器人返回任务表格草稿"]
    C --> D{"协调人确认？"}
    D -->|修改| B
    D -->|确认上传| E["写入任务总表并追加更新记录"]
    E --> F["群内执行、讨论和汇报进展"]
    F --> G["成员直接修改主表或在群里 @机器人"]
    G --> H["Workflow 或机器人自动记录变更"]
    H --> I["定时读取动态视图并生成周报"]
    I --> J["按设置先私聊预览或直接发群"]
```
...
1. **任务进入群聊**：老师或成员在群里提出安排，让任务背景和责任关系对所有人可见。
2. **私聊整理草稿**：协调人把零散消息交给机器人，机器人先整理成表格草稿，不直接上传，也不直接发群。
3. **确认后写入**：协调人补充负责人、截止时间、交付物、研究计划和需老师确认点，明确说“上传”后才写入任务总表。
4. **成员执行和更新**：成员可以直接编辑任务总表，也可以在群里 `@机器人` 汇报进展。普通进度可直接整理；新建、删除、完成、负责人或截止时间变更等高影响操作需要确认。
5. **自动形成记录**：成员手动修改主表时，由 Workflow 写入更新记录；机器人或 CLI 修改时，由 Workflow 或机器人在同一次操作中写入更新记录。普通成员不要直接编辑更新记录表。
6. **集中处理老师确认**：需要老师决定的问题写成具体问句，放入“卡点/需老师确认”，并进入待老师确认视图；确认后把结论写回研究计划或最新进展并清空确认点。
7. **周报前维护日期**：成员平时持续更新进度，并在周报生成前检查当前阶段截止时间。本周任务只包含截止日期明确位于本周且尚未完成的任务。
8. **周报按三个视图生成**：机器人读取“本周任务”“待老师确认”“近四周已完成”。三个数据源全部成功后才生成；根据团队设置，先私聊协调人预览，或直接发送到群里。

私聊机器人时可以这样说：

```text
请把下面的安排整理成任务表格草稿，先不要上传。我确认后再写入看板。
```
