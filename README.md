...
当群成员 `@机器人` 汇报一个已有任务的进展时，机器人进入“最小拟更新”模式，不把一句日常汇报扩展成完整任务讨论。
...
1. 先匹配唯一的已有任务；只有无法唯一匹配时才询问任务名称。
2. 最新进展只整理成员明确说出的事实，不补充解释、下一步、样品数量、卡点或新问题。
3. 明确说“完成/已完成/做完”时拟改为“已完成”；说“进行中/已送样/结果已返回/正在分析”等时拟改为“进行中”；没有状态线索时保持原状态。
4. 没有明确提到需要老师决定时，不新增、不清空也不修改“卡点/需老师确认”，更不把状态改为“待老师确认”。
5. 拟更新后只问“是否确认上传看板？”。确认必须针对本次预览，并来自同一对话或消息线程；预览有变化就重新确认。
6. 确认后只修改预览中的状态、最新进展和内部更新来源，并同步追加关联更新记录，不顺带修改其他字段。
7. 如果工具或权限不可用，机器人必须明确说“本次尚未写入看板”并指出缺少的访问层，不能假装成功。
...
- 机器人在目标群聊中。
- 机器人具备读取群消息和发送群消息的权限。
- 已识别机器人真正执行写入的身份；不要默认它就是当前飞书 CLI 应用或群里显示的机器人身份。
- 该执行身份具备应用/API 授权、多维表格协作者权限，以及高级权限中的任务总表编辑和更新记录追加权限。
- 机器人运行平台另行配置了最小工具白名单；多维表格分享权限不会自动开放智能体工具。
- 对新建任务、修改负责人、修改截止时间、标记完成、删除任务等关键操作，仍建议先向人类负责人确认。

普通群成员触发的进度更新，建议只开放：读取/检索已有任务、确认后修改状态和最新进展、写内部更新来源、追加关联更新记录、同步状态快照并清空更新来源。默认不开放新建/删除任务、负责人、截止时间、需老师确认、其他字段、视图、Workflow 和权限管理；确需开放时，应建立单独的授权流程。

配置完成后，必须从群聊真实触发一次“读取任务 -> 返回拟更新 -> 同线程确认 -> 写入任务 -> 追加更新记录”的完整测试。只用 CLI 账号测试成功，不能证明群机器人已经可用。
